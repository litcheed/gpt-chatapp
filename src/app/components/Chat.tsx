...
const Chat = () => {
  ...
  const sendMessage = async () => {
    ...
    //メッセージをFireStoreに保存
    const roomDocRef = doc(db, "rooms", "4rB1wysnu9cb78Btfr5w");
    const messaageCollectionRef = collection(roomDocRef, "messages");
    await addDoc(messaageCollectionRef, messageData);
  };
        ...
        {messages.map((message, index) => (
            <div key={index} className={message.sender === "user" ? "text-right" : "text-left"}>
              <div
               ...
               ? 'bg-white inline-block rounded px-4 py-2 shadow-[3px_3px_0px_0px_rgba(100,100,100)]'
               : 'bg-gray-700 inline-block rounded px-4 py-2 shadow-[3px_3px_0px_0px_rgba(100,100,100)]'}
              >
